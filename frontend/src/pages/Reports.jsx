...
function normalizeReports(raw) {
  ...
  // If the API wraps everything inside raw.data
  const d = raw?.data ?? raw;
  return {
    monthly:            toArray(d?.monthly),
    avgMonthlyIncome:   d?.avgMonthlyIncome   ?? 0,
    avgMonthlyExpenses: d?.avgMonthlyExpenses  ?? 0,
    savingsRate:        d?.savingsRate         ?? 0,
    topCategories:      toArray(d?.topCategories),
  };
}
...
export default function Reports() {
  ...
  useEffect(() => {
    setLoading(true);
    getReports(months)
      .then(r => setData(normalizeReports(r)))
      .catch(e => { console.error('[Reports] fetch error:', e); setData(DEFAULT_DATA); })
            ...
            {PERIOD_OPTIONS.map(o => (
              <button key={o.value} onClick={() => setMonths(o.value)}
                className={`px-3 md:px-4 py-1.5 rounded-lg text-xs md:text-sm font-medium transition-all ${
                  months === o.value ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}>
                {o.label}
              </button>
            ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mb-5">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="font-mono font-bold text-xl md:text-2xl text-slate-800 mt-1">
                    {currency}{Math.round(data.avgMonthlyIncome).toLocaleString()}
                  </p>
                </div>
                <div className="w-10 h-10 md:w-12 md:h-12 bg-green-50 rounded-xl flex items-center justify-center text-green-500">
                  <TrendingUp size={18} />
                </div>
              </div>
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="font-mono font-bold text-xl md:text-2xl text-slate-800 mt-1">
                    {currency}{Math.round(data.avgMonthlyExpenses).toLocaleString()}
                  </p>
                </div>
                <div className="w-10 h-10 md:w-12 md:h-12 bg-red-50 rounded-xl flex items-center justify-center text-red-400">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-emerald-500 mt-1 flex items-center gap-1">
                    <TrendingUp size={11} />{savingsLabel}
                  </p>
                </div>
                <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-50 rounded-xl flex items-center justify-center text-blue-400">
                  <Activity size={18} />
                </div>
              </div>
            </div>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={data.monthly} margin={{ left: -15, right: 5 }}>
                      ...
                      <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false}
                        tickFormatter={v => `${currency}${v}`} width={50} />
                    ...
                    <div key={cat.name ?? i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-slate-700 rounded-full transition-all duration-500"
                          style={{ width: `${((cat.total ?? 0) / maxCat) * 100}%` }} />
                      </div>
                      <span className="font-mono text-xs md:text-sm font-semibold text-slate-700 w-20 text-right flex-shrink-0">
                        {currency}{(cat.total ?? 0).toFixed(0)}
                      </span>
                    </div>
                  ))}
                </div>
