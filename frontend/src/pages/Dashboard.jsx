...
const categoryEmojis = {
  ...
  Healthcare: '⚕️', Shopping: '🛍️', Utilities: '💡', Education: '📚',
  Housing: '🏠', Investments: '📈',
};
...
export default function Dashboard({ onNav }) {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      // Summary object
      const rawSummary = s?.data;
      setSummary(
        rawSummary?.data ?? rawSummary ??
        { totalIncome: 0, totalExpenses: 0, totalBalance: 0, totalSavings: 0 }
      );

      setMonthly(toArray(m?.data));
      setPieData(toArray(c?.data));
      setTransactions(toArray(t?.data));
      setAllCategories(toArray(cats?.data));
  ...
  const handleAddTransaction = async (data) => {
    const { createTransaction } = await import('../utils/api');
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 md:p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-5">
          <StatCard title="Balance"  value={summary?.totalBalance  ?? 0} currency={currency} icon={<Wallet size={18} />}       color="blue"   delay="fade-up-1" />
          <StatCard title="Income"   value={summary?.totalIncome   ?? 0} currency={currency} icon={<TrendingUp size={18} />}   color="green"  delay="fade-up-2" />
          <StatCard title="Expenses" value={summary?.totalExpenses ?? 0} currency={currency} icon={<TrendingDown size={18} />} color="red"    delay="fade-up-3" />
          <StatCard title="Savings"  value={summary?.totalSavings  ?? 0} currency={currency} icon={<PiggyBank size={18} />}   color="purple" delay="fade-up-4" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          ...
          <div className="md:col-span-2 bg-white rounded-2xl p-4 md:p-5 shadow-sm border border-slate-100">
            <h2 className="font-bold text-slate-800 mb-4 text-sm md:text-base">
              Income vs Expenses{' '}
              <span className="text-slate-400 font-normal text-xs">(Last 6 Months)</span>
            </h2>
            {monthly.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-1.5 mt-2">
                  {pieData.slice(0, 4).map((cat, i) => (
                    <div key={cat.name ?? i} className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono font-medium text-slate-700">
                        {currency}{(cat.value ?? 0).toFixed(0)}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between px-4 md:px-5 py-4 border-b border-slate-100">
            ...
            <button
              onClick={() => onNav && onNav('transactions')}
          ...
          {transactions.length === 0 ? (
            ...
            <div className="divide-y divide-slate-50">
              {transactions.map(t => (
                <div
                  key={t._id}
                  className="flex items-center justify-between px-4 md:px-5 py-3 hover:bg-slate-50/60 transition-colors group"
                >
