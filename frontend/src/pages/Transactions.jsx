            ...
            <table className="w-full min-w-[500px]">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left px-4 md:px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">Date</th>
                  <th className="text-left px-4 md:px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">Description</th>
                  <th className="text-left px-4 md:px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">Category</th>
                  <th className="text-right px-4 md:px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">Amount</th>
                  <th className="text-right px-4 md:px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  <tr key={t._id} className="hover:bg-slate-50/60 transition-colors group">
                    <td className="px-4 md:px-6 py-3.5 text-xs md:text-sm text-slate-500 whitespace-nowrap">
                      {new Date(t.date).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })}
                    </td>
                    <td className="px-4 md:px-6 py-3.5 text-xs md:text-sm font-semibold text-slate-800 max-w-[140px] truncate">{t.description}</td>
                    <td className="px-4 md:px-6 py-3.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${TYPE_COLORS[t.category] || 'bg-slate-100 text-slate-600'}`}>
                        {t.category}
                      </span>
                    </td>
                    <td className={`px-4 md:px-6 py-3.5 text-xs md:text-sm font-mono font-semibold text-right whitespace-nowrap ${t.type === 'income' ? 'text-emerald-600' : 'text-slate-700'}`}>
