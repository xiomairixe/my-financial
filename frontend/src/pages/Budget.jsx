      ...
      <div className="p-8 flex-1">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-slate-700 w-36 text-center">{monthLabel}</span>
          ...
          <div className="flex gap-8 text-sm">
            <div className="text-right">
              <p className="text-slate-400">Total Budgeted</p>
              <p className="font-mono font-bold text-slate-700">${data.totalBudgeted.toFixed(2)}</p>
            </div>
            <div className="text-right">
              <p className="text-slate-400">Total Spent</p>
              <p className="font-mono font-bold text-slate-700">${data.totalSpent.toFixed(2)}</p>
            </div>
            <div className="text-right">
              <p className="text-slate-400">Remaining</p>
              <p className={`font-mono font-bold ${data.remaining >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>${data.remaining.toFixed(2)}</p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 flex flex-col items-center gap-3">
            ...
            <p className="font-bold text-slate-700">No budgets set for this month</p>
            <p className="text-sm text-slate-400 text-center">Create budgets for your spending categories to track your<br />expenses and save more money.</p>
          ...
          <div className="grid grid-cols-1 gap-4">
            {data.budgets.map(b => {
              ...
              return (
                <div key={b._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 group">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-sm font-mono font-bold ${over ? 'text-red-500' : 'text-emerald-500'}`}>
                        ${Math.abs(b.monthlyLimit - b.spent).toFixed(2)} {over ? 'over' : 'left'}
                      </span>
                  ...
                  <div className="flex justify-between mt-1">
                    ...
                    <span className="text-xs text-slate-400">Budget: ${b.monthlyLimit.toFixed(0)}</span>
                  </div>
                </div>
              );
            })}
