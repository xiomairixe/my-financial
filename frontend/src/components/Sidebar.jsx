...
// ← TINANGGAL ang internal useState — ginagamit na ang props mula sa App.jsx
export default function Sidebar({ active, onNav, user, collapsed, onCollapse }) {
  ...
  return (
    <aside
      ...
      className="fixed left-0 top-0 h-full bg-slate-900 flex flex-col transition-all duration-300 z-50"
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-800">
        ...
        {!collapsed && (
          <span className="font-bold text-white text-lg transition-all duration-300">FinTrack</span>
        )}
      </div>
      ...
      <nav className="flex flex-col gap-1 flex-1 px-3 py-4 overflow-y-auto">
        {navItems.map(({ icon: Icon, label, id }) => (
          <button key={id} onClick={() => onNav(id)}
            title={collapsed ? label : undefined} // ← tooltip kapag collapsed
      ...
      <button
        onClick={() => onCollapse(c => !c)} // ← onCollapse na, hindi setCollapsed
